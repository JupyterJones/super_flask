...
app.config['UPLOADFOLDER'] = 'static/archived_resources'
app.config['PROCESSEDFOLDER'] = 'static/archived_resources'
...
def rorschach():
    ...
    for count in range(2): # Generate 2 inkblots as an example
        ...
        inkblot_image = processr_image(seed_count, seed_max_size, count=count)
...
def process_selected_images():
    ...
    shutil.copy(output_mp4_file, 'static/temp_exp/titleX.mp4')
...
def create_mp4_from_images(images_list, output_file, fps):
    ...
    shutil.copy(output_file, 'static/temp_exp/titleX.mp4')
...
def concat_videos():
    ...
    command = [
        'ffmpeg', '-hide_banner', '-loglevel', 'info',  # Enable FFmpeg logit for debugging
        '-i', title_path,
        '-i', main_path,
...
def saveimage():
    try:
        ...
        processed_img_path = os.path.join(app.config['PROCESSEDFOLDER'], processed_image)

        # Move the processed image to final storage if necessary
        # For now, it's already in the `archived_resources` folder

        logit(f"Image {processed_image} saved successfully")

        return redirect(url_for('select_image'))
    
    except Exception as e:
        logit(f"Error saving image: {e}")
        return redirect(url_for('select_image'))
